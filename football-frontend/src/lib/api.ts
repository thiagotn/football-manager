...
export type GroupMember = { id: string; player: PlayerMemberView; role: 'admin' | 'member'; skill_stars: number | null; position: string | null; nickname: string | null; created_at: string };
...
export const groups = {
  ...
  update: (id: string, data: { name?: string; description?: string; per_match_amount?: number | null; monthly_amount?: number | null; recurrence_enabled?: boolean; is_public?: boolean; voting_enabled?: boolean; vote_open_delay_minutes?: number; vote_duration_hours?: number; timezone?: string; team_slots?: TeamSlot[] | null }) => patch<Group>(`/groups/${id}`, data),
...
export type VoteTop5ResultItem = { position: number; player_id: string; name: string; nickname: string | null; avatar_url: string | null; points: number };
export type VoteFlopResultItem = { player_id: string; name: string; nickname: string | null; avatar_url: string | null; votes: number };
